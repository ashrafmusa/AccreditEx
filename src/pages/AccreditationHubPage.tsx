...
const AccreditationHubPage: React.FC<AccreditationHubPageProps> = ({ setNavigation }) => {
    ...
    const handleImportPrograms = (programs: AccreditationProgram[], mode: 'add' | 'replace') => {
        try {
            if (mode === 'replace') {
            ...
            toast.success(
                `${t('successfully')} ${t('imported')} ${programs.length} ${t('programs').toLowerCase()}`
            );
    ...
    return (
        <div className="space-y-6">
            {canModify && (
                <div className="flex flex-col sm:flex-row gap-3 justify-between items-start sm:items-center">
                    <div className="flex flex-col sm:flex-row gap-3 flex-1">
                        <button onClick={() => { setEditingProgram(null); setIsModalOpen(true); }} className="bg-brand-primary text-white px-5 py-2.5 rounded-lg hover:bg-indigo-700 flex items-center justify-center font-semibold shadow-sm">
            ...
            {!canModify && <RestrictedFeatureIndicator featureName="Accreditation Programs Management" />}
