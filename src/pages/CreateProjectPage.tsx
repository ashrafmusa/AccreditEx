...
const CreateProjectPage: React.FC<CreateProjectPageProps> = ({ navigation, setNavigation }) => {
  ...
  const { accreditationPrograms, projectTemplates, getTemplatesByProgram } = useAppStore();
  ...
  const validateForm = (): boolean => {
    ...
    if (!name || name.trim().length === 0) {
      ...
      newErrors.name = t('projectNameMinLength') || 'Project name must be at least 3 characters';
    } else if (name.trim().length > 200) {
      newErrors.name = t('projectNameMaxLength') || 'Project name must not exceed 200 characters';
    }
    ...
    if (endDate && startDate && endDate < startDate) {
      newErrors.endDate = t('endDateMustBeAfterStart') || 'End date must be after start date';
    }
    
    if (description && description.length > 1000) {
      newErrors.description = t('descriptionMaxLength') || 'Description must not exceed 1000 characters';
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditMode && existingProject) {
        ...
        toast.success(t('projectUpdatedSuccessfully') || 'Project updated successfully');
      ...
      const errorMsg = error instanceof Error ? error.message : t('failedToSaveProject') || 'Failed to save project';
      ...
      <form onSubmit={handleSubmit} className="space-y-6 bg-brand-surface dark:bg-dark-brand-surface p-8 rounded-lg shadow-md border border-brand-border dark:border-dark-brand-border animate-fadeIn">
        ...
        {Object.keys(errors).length > 0 && (
          <div className="p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg">
            <h3 className="text-red-800 dark:text-red-300 font-semibold mb-2">{t('pleaseFixErrors') || 'Please fix the errors below'}</h3>
            <ul className="list-disc list-inside space-y-1">
              {Object.entries(errors).map(([field, message]) => (
                <li key={field} className="text-red-700 dark:text-red-400 text-sm">{message}</li>
              ))}
            </ul>
          </div>
        )}
        ...
        <div>
            ...
            <textarea 
              ...
              rows={4} 
              className={`${inputClasses} ${errors.description ? 'border-red-500 focus:ring-red-500' : ''}`}
              maxLength={1000}
            />
            {errors.description && <p className="text-red-500 text-sm mt-1">{errors.description}</p>}
            <p className="text-gray-400 text-xs mt-1">{description.length}/1000</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <select 
                  ...
                  className={`${inputClasses} ${errors.programId ? 'border-red-500 focus:ring-red-500' : ''}`}
                  required 
                  disabled={isEditMode}
                >
            ...
            <div>
                ...
                <select 
                  ...
                  className={`${inputClasses} ${errors.leadId ? 'border-red-500 focus:ring-red-500' : ''}`}
                  required
                >
                    <option value="">{t('selectLead')}</option>
                    {users.map(u => <option key={u.id} value={u.id}>{u.name}</option>)}
                </select>
                {errors.leadId && <p className="text-red-500 text-sm mt-1">{errors.leadId}</p>}
            </div>
        </div>
        ...
        <div className="flex justify-end gap-4 pt-4 border-t border-brand-border dark:border-dark-brand-border">
            <button 
              ...
              className="bg-white dark:bg-gray-700 py-2 px-4 border border-gray-300 dark:border-gray-500 rounded-md shadow-sm text-sm font-medium hover:bg-gray-50 dark:hover:bg-gray-600 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {t('cancel')}
            </button>
            <button 
              type="submit" 
              disabled={isSubmitting}
              className="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-brand-primary hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed"
            >
