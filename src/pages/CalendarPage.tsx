...
const CalendarPage: React.FC<CalendarPageProps> = ({ setNavigation }) => {
  ...
  const { addCustomEvent, updateCustomEvent, deleteCustomEvent } =
    useAppStore();
  ...
  const handleSaveEvent = (
    eventData: Omit<UnifiedEvent, "id" | "type" | "color" | "link">,
    id?: string,
  ) => {
    if (id) {
      updateCustomEvent({ ...eventData, id, type: "Custom" } as any);
    } else {
      addCustomEvent(eventData as any);
    }
