...
const DepartmentsPage: React.FC<DepartmentsPageProps> = (props) => {
  ...
  const handleSave = async (deptData: Department | Omit<Department, 'id'>) => {
    try {
      if ('id' in deptData) {
        ...
        showToast(t('departmentUpdatedSuccessfully') || 'Department updated successfully', 'success');
      } else {
        await onCreateDepartment(deptData);
        showToast(t('departmentCreatedSuccessfully') || 'Department created successfully', 'success');
      }
      setIsModalOpen(false);
    } catch (error) {
      const errorMsg = error instanceof Error ? error.message : t('failedToSaveDepartment') || 'Failed to save department';
  ...
  const handleDelete = async (deptId: string, deptName: string) => {
    ...
    const confirmMsg = `${t('areYouSureDeleteDepartment') || 'Are you sure you want to delete this department?'} "${deptName}" (${usersInDept} ${t('staffMembers')})?`;
    ...
    try {
      ...
      showToast(t('departmentDeletedSuccessfully') || 'Department deleted successfully', 'success');
    } catch (error) {
      const errorMsg = error instanceof Error ? error.message : t('failedToDeleteDepartment') || 'Failed to delete department';
      showToast(errorMsg, 'error');
  ...
  return (
    <div className="space-y-6">
      ...
      {!canModify && <RestrictedFeatureIndicator featureName="Department Management" />}
