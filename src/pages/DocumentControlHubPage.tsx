...
interface DocumentControlHubPageProps {
  ...
  onAddProcessMap: (data: { name: { en: string; ar: string }, tags?: string[], category?: string, departmentIds?: string[] }) => void;
  onDeleteDocument: (docId: string) => void;
  onApproveDocument: (docId: string) => void;
}

const DocumentControlHubPage: React.FC<DocumentControlHubPageProps> = ({ documents, standards, departments, currentUser, onUpdateDocument, onCreateDocument, onAddProcessMap, onDeleteDocument, onApproveDocument }) => {
  ...
  const handleConfirmSignature = async () => {
    ...
    try {
      ...
      toast.success(t('documentApprovedSuccessfully') || 'Document approved successfully');
      setSigningDoc(null);
    } catch (error) {
      const errorMsg = error instanceof Error ? error.message : t('failedToApproveDocument') || 'Failed to approve document';
  ...
  const handleSaveDocument = async (doc: AppDocument) => {
    ...
    try {
      ...
      toast.success(t('documentSavedSuccessfully') || 'Document saved successfully');
      setViewingDoc(null);
    } catch (error) {
      const errorMsg = error instanceof Error ? error.message : t('failedToSaveDocument') || 'Failed to save document';
      toast.error(errorMsg);
  ...
  const handleDeleteDocument = async (docId: string) => {
    ...
    if (!window.confirm(`${t('areYouSureDeleteDocument') || 'Are you sure you want to delete this document?'} "${docName}"?`)) {
      return;
    }
  ...
  const handleCreateDocument = async (docData: any) => {
    try {
      ...
      toast.success(t('documentCreatedSuccessfully') || 'Document created successfully');
      setIsMetaModalOpen(false);
    } catch (error) {
      const errorMsg = error instanceof Error ? error.message : t('failedToCreateDocument') || 'Failed to create document';
      toast.error(errorMsg);
  ...
  const handleAddProcessMap = async (mapData: any) => {
    try {
      if (!mapData.name?.en || !mapData.name?.en.trim()) {
        toast.error(t('processMapNameRequired') || 'Process map name is required');
        return;
      }
      ...
      toast.success(t('processMapCreatedSuccessfully') || 'Process map created successfully');
      setIsProcessMapModalOpen(false);
    } catch (error) {
      const errorMsg = error instanceof Error ? error.message : t('failedToCreateProcessMap') || 'Failed to create process map';
      toast.error(errorMsg);
  ...
  const controlledDocuments = useMemo(() => {
    ...
    if (activeCategory === 'my_documents') {
        ...
        filtered = filtered.sort((a, b) => new Date(b.uploadedAt).getTime() - new Date(a.uploadedAt).getTime()).slice(0, 10);
    ...
    if (activeFilters.author) {
        filtered = filtered.filter(doc => doc.uploadedBy?.toLowerCase().includes(activeFilters.author!.toLowerCase()));
    }
    if (activeFilters.category) {
        filtered = filtered.filter(doc => doc.category === activeFilters.category);
    }
    if (activeFilters.departmentId) {
        filtered = filtered.filter(doc => doc.departmentIds?.includes(activeFilters.departmentId!));
    }
  ...
  const documentCounts = useMemo(() => {
      const counts: Record<string, number> = { all: 0, my_documents: 0, recent: 0 };
      const controlled = documents.filter(d => d.isControlled);
      counts.all = controlled.length;
      
      // Count by type
      ['Policy', 'Procedure', 'Process Map', 'Evidence', 'Report'].forEach(type => {
          counts[type] = controlled.filter(d => d.type === type).length;
      });

      // Count by category
      counts.my_documents = controlled.filter(doc => doc.uploadedBy === currentUser.name).length;
      counts.recent = Math.min(10, controlled.length);
      
      // Count by department
      departments.forEach(dept => {
          counts[dept.id] = controlled.filter(doc => doc.departmentIds?.includes(dept.id)).length;
      });
  ...
  return (
    <div className="space-y-6">
            ...
            <div className="relative">
                ...
                {isAddMenuOpen && (
                    <div className="absolute top-full right-0 mt-2 w-48 bg-white dark:bg-dark-brand-surface rounded-md shadow-lg border dark:border-dark-brand-border z-10">
                        ...
                        <button onClick={() => {setIsProcessMapModalOpen(true); setIsAddMenuOpen(false);}} className="w-full text-left px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-700">{t('processMap')}</button>
                    </div>
                )}
            </div>
        )}
      ...
      {!canModify && <RestrictedFeatureIndicator featureName="Document Management" />}
      ...
      <div className="flex gap-6">
          ...
          <div className="flex-1 space-y-6 min-w-0">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <StatCard title={t('totalDocuments')} value={stats.total} icon={DocumentTextIcon} color="from-blue-500 to-blue-700 bg-gradient-to-br" />
                ...
                <StatCard title={t('drafts')} value={stats.drafts} icon={PencilIcon} color="from-gray-500 to-gray-700 bg-gradient-to-br" />
            </div>
      ...
      <ProcessMapMetadataModal isOpen={isProcessMapModalOpen} onClose={() => setIsProcessMapModalOpen(false)} onSave={handleAddProcessMap} />

      {/* Document Metadata Modal */}
      <DocumentMetadataModal isOpen={isMetaModalOpen} onClose={() => setIsMetaModalOpen(false)} onSave={handleCreateDocument} />
