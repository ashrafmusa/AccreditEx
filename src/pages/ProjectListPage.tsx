...
const ProjectListPage: React.FC<ProjectListPageProps> = ({ setNavigation }) => {
  ...
  const handleBulkArchive = async () => {
    if (!window.confirm(`${t('areYouSureArchive')} ${selectedProjects.length} ${t('project' + (selectedProjects.length > 1 ? 's' : ''))}?`)) {
      return;
    }
    try {
      ...
      const errorMsg = error instanceof Error ? error.message : t('failedToArchiveProjects');
  ...
  const handleBulkRestore = async () => {
    if (!window.confirm(`${t('areYouSureRestore')} ${selectedProjects.length} ${t('project' + (selectedProjects.length > 1 ? 's' : ''))}?`)) {
      return;
    }
    try {
      ...
      const errorMsg = error instanceof Error ? error.message : t('failedToRestoreProjects');
      toast.error(errorMsg);
  ...
  const handleBulkDelete = async () => {
    if (!window.confirm(`${t('areYouSurePermanentlyDelete')} ${selectedProjects.length} ${t('project' + (selectedProjects.length > 1 ? 's' : ''))}? ${t('thisActionCannotBeUndone')}`)) {
      return;
    }
    try {
      ...
      const errorMsg = error instanceof Error ? error.message : t('failedToDeleteProjects');
      toast.error(errorMsg);
  ...
  const handleBulkUpdateStatus = async (status: ProjectStatus) => {
    if (!window.confirm(`${t('updateStatusTo')} ${t(status.replace(/\s/g, '').toLowerCase() as any) || status} ${t('forProjects')} ${selectedProjects.length}?`)) {
      return;
    }
    try {
      ...
      const errorMsg = error instanceof Error ? error.message : t('failedToUpdateProjects');
      toast.error(errorMsg);
  ...
  const handleDelete = (projectId: string) => {
    if (window.confirm(t('areYouSureDeleteProject'))) {
      try {
        ...
        const errorMsg = error instanceof Error ? error.message : t('failedToDeleteProject');
        toast.error(errorMsg);
