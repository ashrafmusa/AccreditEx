      ...
      <div className="w-full max-w-2xl bg-brand-surface dark:bg-dark-brand-surface rounded-2xl shadow-2xl overflow-hidden animate-[scaleIn_0.5s_ease-out_forwards]">
        ...
        <div className="bg-slate-50 dark:bg-slate-900/50 p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4">
            ...
            >
              {dir === "ltr" ? (
                <ChevronLeftIcon className="w-4 h-4 ltr:mr-2 rtl:ml-2" />
              ) : (
                <ChevronRightIcon className="w-4 h-4 ltr:mr-2 rtl:ml-2" />
              )}
              <span>{t("previous")}</span>
            </Button>

            {isLastStep ? (
              <Button onClick={onComplete} size="lg" className="shadow-lg">
                {t("getStarted")}
              </Button>
            ) : (
              <Button onClick={nextStep}>
                <span>{t("next")}</span>
                {dir === "ltr" ? (
                  <ChevronRightIcon className="w-4 h-4 ltr:ml-2 rtl:mr-2" />
                ) : (
                  <ChevronLeftIcon className="w-4 h-4 ltr:ml-2 rtl:mr-2" />
                )}
              </Button>
            )}
          </div>
