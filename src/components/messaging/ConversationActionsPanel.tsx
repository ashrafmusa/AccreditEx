...
import { CheckBadgeIcon, BellIcon, ArchiveBoxIcon, TrashIcon } from '@/components/icons';
    ...
    <div className="relative">
      ...
      {showMenu && (
        <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 z-10">
          ...
          >
            <CheckBadgeIcon className={`${size === 'sm' ? 'w-4 h-4' : 'w-4 h-4'} ${isPinned ? 'fill-yellow-400 text-yellow-400' : ''}`} />
            <span>{isPinned ? t('unpin') : t('pin')} {t('pinConversation').split(' ').slice(1).join(' ')}</span>
          </button>
          ...
          >
            ...
            <span>{isMuted ? t('unmute') : t('mute')} {t('muteConversation').split(' ').slice(1).join(' ')}</span>
          </button>
          ...
          >
            ...
            <span>{isArchived ? t('unarchive') : t('archive')} {t('archiveConversation').split(' ').slice(1).join(' ')}</span>
          </button>
