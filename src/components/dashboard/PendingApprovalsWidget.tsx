...
const PendingApprovalsWidget: React.FC<PendingApprovalsWidgetProps> = ({
  ...
  const getDaysUntilDue = (dueDate?: string) => {
    ...
    if (diffInDays === 1)
      return { text: t("tomorrow"), color: "text-amber-500" };
    if (diffInDays <= 7)
      return {
        text: `${diffInDays} ${t("daysLeft")}`,
        color: "text-orange-500",
      };
    return {
      text: `${diffInDays} ${t("daysLeft")}`,
      color: "text-brand-text-secondary dark:text-dark-brand-text-secondary",
    };
  };
        ...
        {pendingItems.length > 0 ? (
          pendingItems.map((item) => {
            ...
            return (
              ...
              >
                <div
                  className={`p-2 rounded-lg bg-gray-100 dark:bg-gray-800 ${getTypeColor(item.type)}`}
                >
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    {dueInfo && (
                      <>
                        ...
                        <span
                          className={`text-xs font-medium ${dueInfo.color}`}
                        >
                          {dueInfo.text}
                        </span>
                      </>
