  ...
  return (
    ...
    >
      <div className="flex justify-between items-start">
        ...
        <span
          className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${statusInfo[task.status].color}`}
        >
...
const TeamMemberDashboard: React.FC<DashboardPageProps> = ({
  ...
  const taskStats = useMemo(() => {
    try {
      ...
      const partiallyCompliantTasks = myTasks.filter(
        (t) => t?.status === ComplianceStatus.PartiallyCompliant,
      ).length;
      const nonCompliantTasks = myTasks.filter(
        (t) => t?.status === ComplianceStatus.NonCompliant,
      ).length;
      ...
      const upcomingTasks = myTasks.filter((t) => {
        try {
          if (!t?.dueDate || t.status === ComplianceStatus.Compliant)
            return false;
