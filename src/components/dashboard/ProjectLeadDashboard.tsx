...
import { FolderIcon, CheckCircleIcon, ExclamationTriangleIcon, UsersIcon, ClockIcon, CheckBadgeIcon } from '@/components/icons';
...
const ProjectLeadDashboard: React.FC<DashboardPageProps> = ({ setNavigation }) => {
    ...
    const leadStats = useMemo(() => {
        try {
            ...
            const openCapaReports = myProjects.flatMap(p => p?.capaReports || []).filter(c => c?.status === 'Open');
            ...
            myProjects.forEach(p => {
                try {
                    ...
                    applicableItems.forEach(item => {
                        ...
                        if (item.dueDate) {
                            try {
                                if (new Date(item.dueDate) < new Date() && item.status !== ComplianceStatus.Compliant) {
                                    overdueTaskCount += 1;
                                }
                            } catch (e) {
                                // Skip invalid dates
                    ...
                    console.warn('ProjectLeadDashboard: Error processing project checklist', error);
                }
            });
            ...
            const assignedTasks = myProjects
                ...
                .length;
            const assignmentCoverage = totalTasks > 0 ? ((assignedTasks / totalTasks) * 100).toFixed(1) : '0.0';
