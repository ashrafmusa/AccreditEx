...
const ProgramImportWizardModal: React.FC<ProgramImportWizardModalProps> = ({
  ...
  const [step, setStep] = useState<
    "file-selection" | "review" | "confirmation"
  >("file-selection");
        ...
        try {
          ...
          programs = parsedData.map((prog: any, index: number) => {
            if (!prog.name || !prog.description?.en || !prog.description?.ar) {
              errors.push(
                `${t("row")} ${index + 1}: ${t("missingRequiredFields")}`,
              );
            ...
            if (!prog.name.trim()) {
              errors.push(
                `${t("row")} ${index + 1}: ${t("programNameCannotBeEmpty")}`,
              );
            ...
            if (!prog.description.en.trim() || !prog.description.ar.trim()) {
              errors.push(
                `${t("row")} ${index + 1}: ${t("descriptionCannotBeEmpty")}`,
              );
              throw new Error("Invalid data");
            }
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center flex-1">
            ...
            <div
              className={`flex-1 h-1 ltr:mx-2 rtl:mx-2 ${step === "file-selection" ? "bg-gray-300" : "bg-brand-primary"}`}
            />
            ...
            <div
              className={`flex-1 h-1 ltr:mx-2 rtl:mx-2 ${step === "confirmation" ? "bg-brand-primary" : "bg-gray-300"}`}
            />
        ...
        {step === "file-selection" && (
          <div className="space-y-4">
            ...
            <div
              ...
              className="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg p-8 text-center cursor-pointer hover:border-brand-primary hover:bg-sky-50 dark:hover:bg-gray-700 transition-colors"
            >
              <input
                ref={fileInputRef}
                type="file"
        ...
        {step === "review" && validationResult && (
          <div className="space-y-4 max-h-96 overflow-y-auto">
            ...
            {validationResult.isValid &&
              validationResult.programs.length > 0 && (
                <>
          ...
          <div className="flex gap-2">
            ...
            {step === "file-selection" && (
              <button
                ...
                className="px-4 py-2 bg-brand-primary hover:bg-sky-700 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-lg text-sm font-medium"
              >
