...
const StandardAccordion: React.FC<StandardAccordionProps> = ({
  standard,
  canModify,
  onEdit,
  onDelete,
    ...
    >
      <div className="w-full text-left p-5 focus:outline-none flex justify-between items-start gap-4">
        <div className="flex-grow">
          ...
          <p className="mt-2 text-brand-text-primary dark:text-dark-brand-text-primary text-left">
        ...
        <div className="flex-shrink-0 flex flex-col items-end space-y-2">
          ...
          {canModify && (
            <div className="flex items-center space-x-1 rtl:space-x-reverse">
              <button
                onClick={onEdit}
                className="p-1 text-gray-500 dark:text-gray-400 hover:text-brand-primary rounded-full hover:bg-gray-200 dark:hover:bg-gray-700"
              ...
              <button
                onClick={onDelete}
                className="p-1 text-gray-500 dark:text-gray-400 hover:text-red-600 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700"
        ...
        <div className="px-5 pb-5 border-t border-gray-200 dark:border-dark-brand-border mt-2 pt-4 bg-green-50 dark:bg-green-900/10">
          <div className="space-y-2">
            {standard.documents?.map((doc) => (
              <div
                key={doc.id}
                className="flex items-center justify-between p-2 bg-white dark:bg-gray-700 rounded border border-green-200 dark:border-green-700"
              >
      ...
      {hasSubStandards && isExpanded && (
        <div
          id={`substandards-${standard.standardId}`}
        ...
        >
          <ul className="space-y-3 list-disc ltr:list-inside rtl:list-inside marker:text-brand-primary">
            {standard.subStandards?.map((sub) => (
              ...
              >
                <span className="font-semibold text-brand-text-primary dark:text-dark-brand-text-primary ltr:mr-1 rtl:ml-1">
                  {sub.id}:
                </span>{" "}
                {sub.description}
              </li>
