    ...
    <div className="bg-brand-surface dark:bg-dark-brand-surface rounded-xl shadow-md border border-brand-border dark:border-dark-brand-border hover:shadow-xl hover:-translate-y-1 transition-all flex flex-col justify-between">
      <div className="p-5">
        <div className="flex justify-between items-start">
          <h3 className="text-lg font-bold text-brand-text-primary dark:text-dark-brand-text-primary flex-1 ltr:pr-2 rtl:pl-2">
            {program.title[lang]}
          </h3>
          <span
        ...
        <p className="text-sm text-brand-text-secondary dark:text-dark-brand-text-secondary mt-2 line-clamp-3 h-16">
      ...
      <div className="border-t dark:border-dark-brand-border bg-slate-50 dark:bg-slate-900/50 px-5 py-3 flex justify-end rounded-b-xl">
        <button
          onClick={onAction}
          className="text-sm font-semibold text-brand-primary-600 dark:text-brand-primary-400 hover:underline"
        >
          {status?.status === "Completed" && !status.certificateId
            ? t("retakeQuiz")
            : buttonText}
        </button>
      </div>
