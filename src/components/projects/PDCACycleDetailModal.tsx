...
const PDCACycleDetailModal: React.FC<PDCACycleDetailModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen && activeTab !== "History") {
      generateSuggestions(activeTab as PDCAStage, {
        title:
          type === "cycle"
            ? (cycle as PDCACycle).title
            : (cycle as CAPAReport).description || "",
  ...
  // Helper to get translation key for tab
  const getTabTranslationKey = (tab: PDCAStage | "History"): string => {
    if (tab === "History") return "historyStage";
  ...
  return (
    ...
    >
      <div
        className="bg-white dark:bg-dark-brand-surface rounded-lg shadow-xl w-full max-w-4xl m-4 max-h-[90vh] flex flex-col"
                ...
                className={`px-2 py-0.5 rounded-full text-xs font-medium border ${
                  currentStage === "Plan"
                    ...
                    : currentStage === "Do"
                      ...
                      : currentStage === "Check"
                        ? "bg-rose-100 text-pink-700 border-rose-200"
                        : currentStage === "Act"
            ...
            <div className="lg:col-span-2 space-y-6">
                ...
                <div className="space-y-4">
                  <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg">
                    <h4 className="font-medium mb-2">
                      {t("problemStatement") || "Problem Statement"}
                    </h4>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      rows={4}
                      defaultValue={
                        (cycle as CAPAReport).rootCauseAnalysis || ""
                      }
                ...
                <div className="space-y-6">
                  <div className="bg-white dark:bg-gray-800 border rounded-lg p-4">
                    ...
                    {(cycle as any).improvementMetrics ? (
                      <PDCAMetricsChart
                        metrics={(cycle as any).improvementMetrics}
                      />
                    ) : (
                      <div className="text-center py-8 text-gray-500 text-sm">
                        {t("noMetricsDefined") ||
                          "No metrics defined for this cycle."}
                      </div>
                    )}
                  </div>
                </div>
              ...
              {activeTab === "Act" && (
                <div className="space-y-4">
                  <div>
                    <h4 className="font-medium mb-2">
                      {t("standardization") || "Standardization & Closure"}
                    </h4>
                    <p className="text-sm text-gray-500 mb-2">
                      {t("actStageDescription") ||
                        "Document standardized processes and share lessons learned."}
                    </p>
                    <textarea
                      className="w-full border-gray-300 dark:border-gray-600 rounded-md shadow-sm"
                      rows={4}
              ...
              {activeTab === "History" && (
                <div className="space-y-4">
                  <h4 className="font-medium mb-4">
                    {t("stageHistory") || "Stage History"}
                  </h4>
                  <div className="border-l-2 border-gray-200 dark:border-gray-700 ml-3 space-y-6">
                    {(
                      (cycle as any).stageHistory ||
                      (cycle as CAPAReport).pdcaHistory ||
                      []
                  ...
                  {isLoadingSuggestions ? (
                    ...
                    <ul className="space-y-3">
                      {suggestions.map((suggestion) => (
                        <li
                          key={suggestion.id}
                          className="text-sm bg-white dark:bg-gray-800 p-3 rounded border border-sky-100 dark:border-sky-900 shadow-sm"
                        >
                      ...
                      {suggestions.length === 0 && (
                        <li className="text-sm text-sky-700 dark:text-sky-400 italic">
                          {t("noSuggestions") ||
                            "No suggestions available for this stage."}
                        </li>
                      )}
                    </ul>
                  )}
