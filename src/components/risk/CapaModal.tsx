...
const CapaModal: FC<CapaModalProps> = ({ isOpen, onClose, onSave, users, existingCapa }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center backdrop-blur-sm" onClick={onClose}>
      <div className="bg-white dark:bg-dark-brand-surface rounded-lg shadow-xl w-full max-w-2xl m-4" onClick={e => e.stopPropagation()} dir={dir}>
        <form onSubmit={handleSubmit} className="flex flex-col max-h-[90vh]">
          <div className="p-6 border-b"><h3 className="text-lg font-medium">{t('capaReport')}</h3></div>
          <div className="p-6 space-y-4 overflow-y-auto">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div><label className={labelClasses}>{t('status')}</label><select value={status} onChange={e => setStatus(e.target.value as any)} className={inputClasses}><option value="Open">{t('open')}</option><option value="Closed">{t('closed')}</option></select></div>
              <div>
                <label className={labelClasses}>{t('pdcaStage') || 'PDCA Stage'}</label>
                <select 
                ...
                <p className="text-xs text-gray-500 mt-1">{t('manageStageInBoard') || 'Manage stage in PDCA Board'}</p>
              </div>
            </div>
          </div>
          <div className="bg-gray-50 dark:bg-gray-800 px-6 py-3 flex justify-end gap-3 border-t"><button type="button" onClick={onClose} className="py-2 px-4 border rounded-md">{t('cancel')}</button><button type="submit" className="py-2 px-4 border rounded-md text-white bg-brand-primary">{t('save')}</button></div>
        </form>
      </div>
    </div>
  );
