...
const RiskRegisterTab: React.FC = () => {
  ...
  const handleSave = async (riskData: Risk | Omit<Risk, 'id'>) => {
    try {
      if ('id' in riskData) {
        ...
        showToast(t('riskUpdatedSuccessfully') || 'Risk updated successfully', 'success');
      } else {
        await addRisk(riskData);
        showToast(t('riskCreatedSuccessfully') || 'Risk created successfully', 'success');
      }
      setIsModalOpen(false);
    } catch (error) {
      const errorMsg = error instanceof Error ? error.message : t('failedToSaveRisk') || 'Failed to save risk';
  ...
  const handleDelete = async (riskId: string) => {
    ...
    if (!window.confirm(`${t('areYouSureDeleteRisk') || 'Are you sure you want to delete this risk?'} "${riskTitle}"?`)) {
      return;
    }
    ...
    try {
      ...
      showToast(t('riskDeletedSuccessfully') || 'Risk deleted successfully', 'success');
    } catch (error) {
      const errorMsg = error instanceof Error ? error.message : t('failedToDeleteRisk') || 'Failed to delete risk';
      showToast(errorMsg, 'error');
                  ...
                  <tr key={risk.id}>
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex justify-end items-center gap-2">
                          <button disabled={isDeleting} onClick={() => { setEditingRisk(risk); setIsModalOpen(true); }} className="p-1 hover:text-brand-primary disabled:opacity-50"><PencilIcon className="w-4 h-4"/></button>
                          <button disabled={isDeleting} onClick={() => handleDelete(risk.id)} className="p-1 hover:text-red-600 disabled:opacity-50"><TrashIcon className="w-4 h-4"/></button>
                      </div>
                    </td>
                  </tr>
                )
