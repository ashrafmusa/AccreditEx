...
import { PlusIcon, PencilIcon, TrashIcon, ArrowPathIcon, UploadIcon } from '../icons';
...
const UserCompetencies: React.FC<Props> = ({ user, currentUser, competencies, documents, onUpdateUser }) => {
    ...
    const [uploadingForCompetency, setUploadingForCompetency] = useState<string | null>(null);
        ...
        try {
            ...
            const updatedCompetencies = user.competencies?.map(uc => {
                if (uc.competencyId === uploadingForCompetency) {
                    return {
                        ...uc,
                        evidenceDocumentIds: [...(uc.evidenceDocumentIds || []), documentId]
                    };
                }
                return uc;
            });
                   ...
                   return (
                       <div key={uc.competencyId} className="p-3 rounded-md border dark:border-dark-brand-border flex justify-between items-center hover:bg-gray-50 dark:hover:bg-gray-700/50">
                           <div>
                               ...
                               {/* Evidence Documents */}
                               {uc.evidenceDocumentIds && uc.evidenceDocumentIds.length > 0 && (
            ...
            {showUploader && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm">
                    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-2xl w-full mx-4 p-6">
                        <h3 className="text-lg font-semibold mb-4">{t('uploadCertificate') || 'Upload Certificate'}</h3>
                        ...
                        <div className="mt-6 flex justify-end gap-3">
                            <button
                                onClick={() => { setShowUploader(false); setUploadingForCompetency(null); }}
                                disabled={isUploading}
