...
const SettingsPresetsPanel: React.FC = () => {
  ...
  const { appSettings, setAppSettings, updateAppSettings } = useAppStore();
  ...
  const handleApplyPreset = async (presetId: string) => {
    ...
    try {
      ...
      // Apply preset settings to Firebase and local state
      if (preset.settings && appSettings) {
        const merged = { ...appSettings, ...preset.settings };
        await updateAppSettings(merged);
      }
  ...
  const handleCreatePreset = async () => {
    ...
    try {
      ...
      await auditLog("presets", "createPreset", null, newPreset.name, "create");
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredPresets.map((preset) => {
              const IconComponent =
                PRESET_ICONS[preset.icon || "⭐"] || StarIcon;
