...
const GeneralSettingsPage: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    try {
      // Audit logging: compare old vs new for general settings fields
      const auditFields = [
        "appName",
        "logoUrl",
        "primaryColor",
      ...
      for (const field of auditFields) {
        if (appSettings![field] !== settings[field]) {
          changes[field] = { old: appSettings![field], new: settings[field] };
        }
      }
      if (Object.keys(changes).length > 0) {
        await auditBatch("general", changes);
      }
