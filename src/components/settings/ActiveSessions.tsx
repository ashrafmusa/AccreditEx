...
const ActiveSessions: React.FC<ActiveSessionsProps> = ({ userId }) => {
  ...
  const handleSignOutAllOther = async () => {
    try {
      ...
      success(
        t("signedOutDevices")?.replace("{0}", String(count)) ||
          `Signed out ${count} device(s)`,
      );
  ...
  return (
    <div className="bg-white dark:bg-dark-brand-surface rounded-lg shadow-sm border border-gray-200 dark:border-dark-brand-border">
      ...
      {sessions.length > 0 && (
        <div className="p-4 bg-gray-50 dark:bg-gray-800/50 border-t border-gray-200 dark:border-dark-brand-border">
          <p className="text-xs text-gray-500 dark:text-gray-400">
            {t("sessionsNote") ||
              "Sessions expire after 30 days of inactivity. You can sign out devices you no longer use."}
          </p>
        </div>
      )}
      ...
      {/* Confirm sign-out all other devices */}
      {confirmSignOutAll && (
        <div
          className="fixed inset-0 z-60 flex items-center justify-center bg-black/50 backdrop-blur-sm"
        ...
        >
          ...
          >
            <h4 className="text-sm font-semibold text-gray-900 dark:text-white mb-2">
              {t("signOutAllOther") || "Sign Out All Other Devices"}
            </h4>
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-5">
              {t("confirmSignOutAllOther") ||
                "This will sign you out of all devices except this one. You will stay logged in here."}
            </p>
            <div className="flex gap-3 justify-end">
              <button
                onClick={() => setConfirmSignOutAll(false)}
