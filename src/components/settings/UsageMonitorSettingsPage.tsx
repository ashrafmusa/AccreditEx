  ...
  const handleSave = async () => {
    ...
    try {
      ...
      for (const field of auditFields) {
        if (
          oldValues[field as keyof typeof oldValues] !== usageMonitor[field]
        ) {
          changes.push({
            field,
            oldValue: oldValues[field as keyof typeof oldValues],
            newValue: usageMonitor[field],
          });
        }
      }
      if (changes.length > 0) {
        await auditBatch("usageMonitor", changes);
      }
