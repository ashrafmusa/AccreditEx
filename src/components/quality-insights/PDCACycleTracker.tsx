  ...
  const pdcaStats = {
    ...
    do: allCapas.filter(
      (c) =>
        ...
        c?.rootCause &&
        !c?.effectivenessCheck?.completed,
    ).length,
    check: allCapas.filter(
      (c) =>
      ...
      <div className="relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {stages.map((stage, index) => (
            ...
            >
              <div
                className={`w-16 h-16 rounded-full flex items-center justify-center mb-4 shadow-sm transition-transform group-hover:scale-110 ${stage.color}`}
              >
