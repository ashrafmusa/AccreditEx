...
import { CircleStackIcon, ChevronUpIcon, ChevronDownIcon } from '@/components/icons';
...
const Sparkline: FC<{ data: number[]; color?: string }> = ({ data, color = '#0ea5e9' }) => {
  ...
  return (
    <svg className="w-full h-8 opacity-60" viewBox="0 0 100 100" preserveAspectRatio="none">
      <polyline
        points={points}
        fill="none"
        stroke={color}
...
const TrendIndicator: FC<{ direction: 'up' | 'down' | 'neutral'; value: number; label?: string }> = ({ 
  direction, 
  value, 
  label 
}) => {
  ...
  const Icon = direction === 'up' ? ChevronUpIcon : direction === 'down' ? ChevronDownIcon : null;
...
const StatCard: FC<StatCardProps> = ({ 
  ...
  return (
    <div onClick={onClick} className={`bg-brand-surface dark:bg-dark-brand-surface p-5 rounded-xl shadow-sm border border-brand-border dark:border-dark-brand-border flex flex-col justify-between transition-all hover:shadow-lg hover:-translate-y-1 relative group overflow-hidden ${onClick ? 'cursor-pointer' : ''}`}>
      {/* Background gradient effect */}
      <div className={`absolute inset-0 opacity-0 group-hover:opacity-5 transition-opacity ${color || 'bg-brand-primary'}`} />
      ...
      <div className="relative z-10">
        <div className="flex justify-between items-start mb-3">
          ...
          {Icon && (
            <div className={`text-white p-3 rounded-lg ${color || 'bg-brand-primary'} shadow-md group-hover:scale-110 transition-transform`}>
      ...
      {isLiveLinkable && (
        <div className="absolute top-2 right-2 opacity-50 group-hover:opacity-100 transition-opacity z-20" title={t('liveDataTooltip')}>
