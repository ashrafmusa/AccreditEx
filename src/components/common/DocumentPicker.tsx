...
const DocumentPicker: React.FC<DocumentPickerProps> = ({
  ...
  if (filterType && filterType.length > 0) {
    filteredDocuments = filteredDocuments.filter(doc => filterType.includes(doc.type));
  }
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-3xl w-full mx-4 max-h-[85vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {filteredDocuments.length === 0 ? (
            <div className="text-center py-12 text-gray-500 dark:text-gray-400">
              <p className="text-lg font-medium">{t('noDocumentsAvailable') || 'No documents available'}</p>
              <p className="text-sm mt-2">{t('tryDifferentSearch') || 'Try a different search or filter'}</p>
            </div>
          ) : (
        ...
        <div className="flex items-center justify-between p-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900">
          <div className="text-sm text-gray-600 dark:text-gray-400">
            <span className="font-medium text-brand-primary">{selected.length}</span> {t('selected') || 'selected'}
            {multiSelect && (
              <span className="ml-2">
