...
const EmptyStatePlaceholder: FC<EmptyStatePlaceholderProps> = ({
  ...
  const containerClass = compact ? "py-4 px-4" : "py-8 px-6";
  ...
  return (
    ...
    >
      {Icon && (
        <div
          className={`${iconSize} rounded-full bg-slate-100 dark:bg-slate-700/50 flex items-center justify-center mb-4`}
        >
          <Icon
            className={`${compact ? "w-6 h-6" : "w-8 h-8"} text-brand-text-secondary dark:text-dark-brand-text-secondary`}
          />
        </div>
      )}

      <h3
        className={`${titleSize} font-semibold text-brand-text-primary dark:text-dark-brand-text-primary mb-2`}
      >
        {title}
      </h3>

      <p
        className={`${messageSize} text-brand-text-secondary dark:text-dark-brand-text-secondary max-w-sm mb-4`}
      >
        {message}
      </p>
