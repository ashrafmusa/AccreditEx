...
const UserAvatar: React.FC<UserAvatarProps> = ({
  ...
  // Reset image state when user or avatarUrl changes
  useEffect(() => {
  ...
  const isClickable = onClick
    ? "cursor-pointer hover:ring-2 hover:ring-offset-2 hover:ring-brand-primary transition-all"
    : "";
  ...
  return (
    ...
    >
      ...
      {!hasImage && (
        <div
          className={`w-full h-full flex items-center justify-center font-bold text-white ${sizeClass.text}`}
        >
          {initials}
        </div>
      )}
      ...
      {showStatus && (
        <div
          className={`absolute -bottom-1 -right-1 ${sizeClass.status} bg-green-500 rounded-full ring-2 ring-white dark:ring-gray-800 flex items-center justify-center`}
        >
          <CheckCircleIcon
            className={`${
              size === "xs" || size === "sm"
