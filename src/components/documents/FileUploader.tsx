      ...
      if (rejectedFiles.length > 0) {
        const errors = rejectedFiles.map((rejected) => {
          ...
          if (error.code === "file-too-large") {
            return `${rejected.file.name}: ${t("fileSizeExceeds") || "File size exceeds"} ${formatFileSize(
              maxFileSize,
            )}`;
          } else if (error.code === "file-invalid-type") {
            return `${rejected.file.name}: ${t("fileTypeNotSupported") || "File type not supported"}`;
          } else if (error.code === "too-many-files") {
            return `${t("maxFilesAllowed") || `Maximum ${maxFiles} files allowed`}`;
          }
        ...
        {isDragActive ? (
          ...
          <div className="space-y-2">
            ...
            <div className="text-xs text-gray-400 dark:text-gray-500 mt-3">
              ...
              <p>
                {t("maxSize") || "Max size"}: {formatFileSize(maxFileSize)}{" "}
                {t("perFile") || "per file"}
              </p>
              {multiple && (
                <p>
            ...
            >
              ...
              {!disabled && (
                <button
                  ...
                  className="ms-3 p-1.5 text-gray-400 hover:text-red-600 dark:hover:text-red-400 transition-colors"
