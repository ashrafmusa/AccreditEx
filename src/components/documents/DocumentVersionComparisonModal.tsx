...
const DocumentVersionComparisonModal: React.FC<DocumentVersionComparisonModalProps> = ({
  isOpen,
  onClose,
  document,
  version1,
  ...
  const getVersionContent = (version: number | 'current') => {
    ...
    const historyItem = document.versionHistory?.find(v => v.version === version);
    return {
      version: version,
  ...
  const leftVersion = useMemo(() => getVersionContent(version1), [version1, document]);
  const rightVersion = useMemo(() => getVersionContent(version2), [version2, document]);
        ...
        <header className="p-4 border-b dark:border-dark-brand-border flex justify-between items-center flex-shrink-0">
          <div>
            <h3 className="text-xl font-semibold dark:text-dark-brand-text-primary">
              {t('compareVersions') || 'Compare Versions'}: {document.name[lang]}
            </h3>
        ...
        <main className="flex-grow flex overflow-hidden">
          ...
          <div className="flex-1 flex flex-col border-r dark:border-dark-brand-border">
            ...
            <div className="flex-1 overflow-y-auto p-6">
              <div 
                ...
                dangerouslySetInnerHTML={{ __html: (leftVersion.content && leftVersion.content[lang]) || '' }}
              />
            </div>
          </div>
          ...
          <div className="flex-1 flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-6">
              <div 
                className="prose dark:prose-invert max-w-none" 
                dangerouslySetInnerHTML={{ __html: (rightVersion.content && rightVersion.content[lang]) || '' }}
              />
            </div>
          </div>
        </main>
