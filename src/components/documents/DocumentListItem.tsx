...
const DocumentListItem: React.FC<DocumentListItemProps> = ({
  ...
  const fileExtension = document.fileUrl ? getFileExtension(document.fileUrl) : null;
      ...
      <div className="flex items-start gap-3 flex-1 min-w-0">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 mt-1 flex-wrap">
            ...
            {showStatus && (
              <span className={`text-xs px-2 py-0.5 rounded ${statusColors[document.status]}`}>
                {t(document.status.toLowerCase().replace(' ', '_')) || document.status}
              </span>
            )}
          </div>
