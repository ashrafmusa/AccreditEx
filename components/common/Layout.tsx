...
const Layout: React.FC<LayoutProps> = ({ navigation, setNavigation, children }) => {
  ...
  useEffect(() => {
    ...
    const fetchNotifications = async () => {
        const userNotifications = await backendService.getNotifications(currentUser.id);
  ...
  return (
    <>
      ...
      <div className={`flex h-screen ${dir === 'rtl' ? 'flex-row-reverse' : ''}`}>
        ...
        />
        <div className={`flex-1 flex flex-col overflow-hidden transition-all duration-300 ${isNavExpanded ? 'sm:ms-64' : 'sm:ms-20'}`}>
          <Header 
            navigation={navigation} 
