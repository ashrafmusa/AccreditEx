...
// Force HMR update
import { Project, User, AppDocument, Standard, AccreditationProgram, NavigationState, CAPAReport } from '@/types';
...
type FilterType = 'all' | 'projects' | 'documents' | 'standards' | 'capa' | 'users';
...
const CommandPalette: FC<CommandPaletteProps> = ({ isOpen, setIsOpen, setNavigation, projects, users, documents, standards, programs }) => {
  ...
  const saveRecentSearch = (term: string) => {
    ...
    localStorage.setItem('accreditex_recent_searches', JSON.stringify(newRecent));
  };
  ...
  const searchResults = useMemo(() => {
    ...
    let results: { type: FilterType; name: string; subtitle?: string; link: () => void; icon: any }[] = [];
    ...
    if (activeFilter === 'all' || activeFilter === 'projects') {
      results.push(...projects
        ...
        .map(p => ({
          ...
          link: () => setNavigation({ view: 'projectDetail', projectId: p.id }),
          icon: FolderIcon
        })));
    }
    ...
    if (activeFilter === 'all' || activeFilter === 'documents') {
      results.push(...documents
        .filter(d => (d.name[lang] || d.name.en).toLowerCase().includes(lowerSearch))
        .map(d => ({
    ...
    if (activeFilter === 'all' || activeFilter === 'standards') {
      results.push(...standards
        ...
        .map(s => ({
          ...
          link: () => setNavigation({ view: 'standards', programId: s.programId }),
    ...
    if (activeFilter === 'all' || activeFilter === 'capa') {
      const allCapas = projects.flatMap(p => p.capaReports.map(c => ({ ...c, projectName: p.name, projectId: p.id })));
      results.push(...allCapas
        .filter(c => c.description.toLowerCase().includes(lowerSearch) || c.id.toLowerCase().includes(lowerSearch))
        .map(c => ({
          type: 'capa' as FilterType,
          name: `CAPA: ${c.id}`,
          subtitle: c.description.substring(0, 50) + '...',
          link: () => setNavigation({ view: 'projectDetail', projectId: c.projectId }),
    ...
    return results;
  }, [searchTerm, activeFilter, projects, users, documents, standards, lang, setNavigation]);
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex justify-center items-start pt-20 backdrop-blur-sm transition-all" onClick={() => setIsOpen(false)}>
      <div className="bg-brand-surface dark:bg-dark-brand-surface rounded-xl shadow-2xl w-full max-w-2xl mx-4 overflow-hidden border border-brand-border dark:border-dark-brand-border flex flex-col max-h-[80vh]" onClick={e => e.stopPropagation()}>
                    ...
                    <li key={index}>
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-sm text-brand-text-primary dark:text-dark-brand-text-primary">{term}</span>
                        </div>
                        <div 
                      ...
                      <li key={index}>
                        ...
                        >
                          ...
                          <span className="text-xs text-brand-text-secondary dark:text-dark-brand-text-secondary bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded">
                            {t(item.type === 'standards' ? 'standard' : item.type)}
                          </span>
                        </button>
                      </li>
                    );
        ...
        <div className="p-3 border-t border-brand-border dark:border-dark-brand-border bg-gray-50 dark:bg-gray-900/50 flex items-center justify-between text-xs text-brand-text-secondary dark:text-dark-brand-text-secondary">
          <div className="flex items-center gap-4">
            <span className="flex items-center gap-1">
              <kbd className="px-1.5 py-0.5 rounded bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 font-sans">↑</kbd>
              <kbd className="px-1.5 py-0.5 rounded bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 font-sans">↓</kbd>
          ...
          <span className="flex items-center gap-1">
            <kbd className="px-1.5 py-0.5 rounded bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 font-sans">Esc</kbd>
            <span>{t('toClose')}</span>
          </span>
        </div>
      </div>
