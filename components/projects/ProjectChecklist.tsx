    ...
    <div className="space-y-4">
      <div className="bg-brand-surface dark:bg-dark-brand-surface p-4 rounded-lg shadow-sm border border-brand-border dark:border-dark-brand-border flex flex-col sm:flex-row gap-4">
        ...
        >
          ...
          {Object.values(ComplianceStatus).map(s => (
            <option key={s} value={s}>{t((s.charAt(0).toLowerCase() + s.slice(1).replace(/\s/g, '')) as any)}</option>
          ))}
        </select>
      </div>
      ...
      <div className="space-y-2">
        {filteredChecklist.map(item => (
          <ChecklistItemComponent key={item.id} item={item} project={project} />
        ))}
        {filteredChecklist.length === 0 && (
          <div className="text-center py-10">
            <p className="text-brand-text-secondary dark:text-dark-brand-text-secondary">{t('noChecklistItemsFound')}</p>
          </div>
        )}
      </div>
    </div>
